...
public enum ConfigKeyEnum implements MainConfigInterface {
    CKE_APP_NAME("app.name",
            new String[]{"当前应用的调用关系写入数据库里的表名后缀",
                    "留空时使用固定表名 jacg_关键字（无后缀，便于跨项目复用与AI查询）",
                    "非空时表名形如 jacg_关键字_app.name（用于多项目共享同一数据库时区分表名）",
                    "不能包含字符-，可使用_"},
            String.class, false, ""),
    ...
    CKE_OUTPUT_ROOT_PATH("output.root.path",
            new String[]{"生成方法完整调用链文件的根目录路径，以\"/\"或\"\\\\\"作为分隔符，末尾是否为分隔符不影响",
                    "默认为当前目录"},
            String.class, false, ""),
    CKE_OUTPUT_DIR_FLAG("output.dir.flag",
            new String[]{"生成方法完整调用链文件的目录名中的标志",
                    "完整目录名使用{app.name}{output.dir.flag}_{当前时间}",
                    "默认为空"},
            String.class, false, ""),
    CKE_OUTPUT_DIR_NAME("output.dir.name",
            new String[]{"生成方法完整调用链文件的目录名",
                    "非空时优先使用本参数作为目录名，忽略 " + CKE_OUTPUT_DIR_FLAG.getKey(),
                    "默认为空，使用 " + CKE_OUTPUT_DIR_FLAG.getKey() + " 参数说明的格式"},
            String.class, false, ""),
    CKE_CHECK_JAR_FILE_UPDATED("check.jar.file.updated",
            new String[]{"生成方法完整调用链文件时，若发现jar文件内容发生变化是否退出生成，true: 退出生成，false: 继续生成"},
            Boolean.class, false, Boolean.FALSE.toString()),
    ...
    CKE_CALL_GRAPH_OUTPUT_DETAIL("call.graph.output.detail",
            new String[]{"生成方法完整调用链文件时的详细程度",
                    OutputDetailEnum.getValidValuesAndDesc(false)},
            String.class, true, OutputDetailEnum.ODE_1.getDetail()),
    CKE_IGNORE_DUP_CALLEE_IN_ONE_CALLER("ignore.dup.callee.in.one.caller",
            new String[]{"生成向下的方法完整调用链文件时，在一个调用方法中出现多次的被调用方法（包含方法调用业务功能数据），是否忽略",
                    "true: 忽略，false: 不忽略"},
            Boolean.class, false, Boolean.FALSE.toString()),
    ...
    CKE_CALL_GRAPH_GEN_JSON_CALLER("call.graph.gen.json.caller",
            new String[]{"生成向下的方法完整调用链文件时，是否输出JSON格式的方法完整调用链文件"},
            Boolean.class, false, Boolean.FALSE.toString()),
    CKE_GEN_CALL_GRAPH_NUM_LIMIT("gen.call.graph.num.limit",
            new String[]{"生成方法完整调用链文件时，每个方法允许生成的方法调用数量限制，默认为0，小于等于0代表不限制，若需要限制请设置大于0的值"},
            Integer.class, false, "0"),
    CKE_GEN_CALL_GRAPH_DEPTH_LIMIT("gen.call.graph.depth.limit",
            new String[]{"生成方法完整调用链文件时，允许生成的方法完整调用链深度限制，默认为0，小于等于0代表不限制，若需要限制请设置大于0的值"},
            Integer.class, false, "0"),
    CKE_CALL_GRAPH_WRITE_TO_FILE("call.graph.write.to.file",
            new String[]{"生成方法完整调用链文件时，是否将调用链数据写入文件",
                    "不能与 " + JACGConstants.CONFIG_KEY_CALL_GRAPH_RETURN_IN_MEMORY + " 同时设置为false"},
            Boolean.class, false, Boolean.TRUE.toString()),
    CKE_CALL_GRAPH_RETURN_IN_MEMORY(JACGConstants.CONFIG_KEY_CALL_GRAPH_RETURN_IN_MEMORY,
            new String[]{"生成方法完整调用链文件时，是否在内存中返回调用链数据",
                    "不能与 " + CKE_CALL_GRAPH_WRITE_TO_FILE.getKey() + " 开关同时设置为false"},
            Boolean.class, false, Boolean.FALSE.toString()),
    CKE_CALL_GRAPH_FILE_SHORT_MODE("call.graph.file.short.mode",
            new String[]{"生成方法完整调用链文件时，文件名是否使用更短的模式，以避免超过Windows文件系统支持的长度",
...
//            new String[]{"生成方法完整调用链文件时，是否需要支持将Spring AOP相关的advice中的方法调用生成到方法完整调用链文件中"},
//            Boolean.class, false, Boolean.FALSE.toString()),
    ;
