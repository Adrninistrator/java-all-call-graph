package com.adrninistrator.jacg.handler.method_arg_generics_type;
...
public class MethodArgGenericsTypeHandler extends BaseHandler {
    ...
    public MethodArgGenericsTypeHandler(DbOperator dbOperator, DbOperWrapper dbOperWrapper) {
    ...
    public MethodArgGenericsTypeInfo queryGenericsTypeInfo(String methodHash) {
        ...
        if (sql == null) {
            sql = "select " + JACGSqlUtil.joinColumns(DC.MAGT_ARG_SEQ, DC.MAGT_TYPE, DC.MAGT_GENERICS_TYPE) +
                    " from " + DbTableInfoEnum.DTIE_METHOD_ARG_GENERICS_TYPE.getTableName(dbOperWrapper.getAppName()) +
