package com.adrninistrator.jacg.handler.signature;
 ...
 */
public class ClassSignatureEi1Handler4Query extends BaseHandler {
    private static final Logger logger = LoggerFactory.getLogger(ClassSignatureEi1Handler4Query.class);

    private final JACGExtendsImplHandler jacgExtendsImplHandler;

    public ClassSignatureEi1Handler4Query(ConfigureWrapper configureWrapper) {
        super(configureWrapper);
        this.jacgExtendsImplHandler = new JACGExtendsImplHandler(dbOperator, dbOperWrapper);
    }

    public ClassSignatureEi1Handler4Query(DbOperator dbOperator, DbOperWrapper dbOperWrapper) {
    ...
    public List<String> queryClassSignatureEi1InfoSimple(String simpleClassName, String upperClassName) {
        ...
        if (JACGUtil.isCollectionEmpty(superOrItfClassNameList)) {
    ...
    private List<Object> querySuperOrItfClassNameList(String simpleClassName) {
        ...
        if (sql == null) {
            sql = "select distinct(" + DC.CSEI1_SUPER_ITF_CLASS_NAME + ")" +
                    " from " + DbTableInfoEnum.DTIE_CLASS_SIGNATURE_EI1.getTableName(dbOperWrapper.getAppName()) +
