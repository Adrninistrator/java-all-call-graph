 ...
 * @date 2025/12/10
 * @description: 方法上的Spring MVC RequestMapping注解格式化类，返回的注解内容为“@注解类名＠{uri}＠{注解属性的JSON字符串}”
 */

/*
 示例如下：
 @org.springframework.web.bind.annotation.RequestMapping＠/test1/test2a＠{"method":{"attributeList":["POST"]},"value":{"attributeList":["/test2a","test2b","test2c"]}}
 由于RequestMapping注解的属性有很多是数组格式，因此输出的JSON结构中会有attributeList属性
*/
public class SpringMvcRequestMappingJsonFormatter extends AbstractAnnotationFormatter {
    private static final Logger logger = LoggerFactory.getLogger(SpringMvcRequestMappingJsonFormatter.class);
    ...
    public String handleAnnotation(String fullMethod, String returnType, String className, String annotationName, Map<String, BaseAnnotationAttribute> attributesMap) {
        ...
        return annotationName + JACGConstants.FLAG_AT + path + JACGConstants.FLAG_AT + JACGJsonUtil.getJsonStr(attributesMap);
    }
