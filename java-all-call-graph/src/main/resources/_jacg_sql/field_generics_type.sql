CREATE TABLE if not exists jacg_field_generics_type_{appName} (
  ...
  simple_generics_type varchar(255) NOT NULL COMMENT '非静态字段类型或其中的泛型类型唯一类名',
  ...
  generics_type varchar(255) NOT NULL COMMENT '非静态字段类型或其中的泛型类型类名',
  class_name varchar(255) NOT NULL COMMENT '完整类名',
  PRIMARY KEY (record_id),
  INDEX idx_fgt_cfs_{appName}(simple_class_name, field_name, type, type_seq),
  INDEX idx_fgt_sgt_{appName}(simple_generics_type)
