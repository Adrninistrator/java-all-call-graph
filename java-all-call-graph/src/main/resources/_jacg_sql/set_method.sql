CREATE TABLE if not exists jacg_set_method_{appName} (
  ...
  field_category varchar(5) NOT NULL COMMENT '字段分类，J:JDK中的类型，C:自定义类型，GJ:集合的泛型类型，只涉及JDK中的类型，GC:集合的泛型类型，涉及自定义类型',
  simple_field_type varchar(255) NULL COMMENT '字段类型唯一类名',
  ...
  INDEX idx_sm_fc_{appName}(field_category),
  INDEX idx_sm_sft_{appName}(simple_field_type),
