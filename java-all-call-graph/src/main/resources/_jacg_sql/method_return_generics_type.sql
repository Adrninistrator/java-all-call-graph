CREATE TABLE IF NOT EXISTS jacg_method_return_generics_type{appName} (
  ...
  simple_generics_type_nad varchar(255) NOT NULL COMMENT '方法返回类型或其中的泛型类型唯一类名（不包含数组标志）',
  ...
  generics_type_nad varchar(255) NOT NULL COMMENT '方法返回类型或其中的泛型类型类名（不包含数组标志）',
  full_method text NOT NULL COMMENT '完整方法（类名+方法名+参数）',
  return_type varchar(255) NOT NULL COMMENT '方法返回类型，包含数组标志',
  PRIMARY KEY (record_id),
  INDEX idx_mrgt_mtt{appName}(method_hash, type, type_seq),
  INDEX idx_mrgt_scn{appName}(simple_class_name(255)),
  INDEX idx_mrgt_sgt{appName}(simple_generics_type_nad)
