CREATE TABLE if not exists jacg_method_call_static_field_{appName} (
  ...
  simple_class_name varchar(255) NOT NULL COMMENT '静态字段所在类唯一类名',
  field_name varchar(200) NOT NULL COMMENT '静态字段名称',
  simple_field_type varchar(255) NOT NULL COMMENT '静态字段类型唯一类名',
  class_name varchar(255) NOT NULL COMMENT '静态字段所在类完整类名',
  field_type varchar(255) NOT NULL COMMENT '静态字段类型',
  PRIMARY KEY (record_id),
  INDEX idx_mcsf_scnfn_{appName}(simple_class_name, field_name)
