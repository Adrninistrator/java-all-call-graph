...
public class CFMCAMethodCallHandler extends BaseHandler {
    ...
    public void addMethodCall() {
        // 根据被调用类名与方法名查询调用方信息
        List<CallGraphCallerInfo> callGraphCallerInfoList = dbOperWrapper.getCallerInfoByCallee(TestCFMCAFlow.class.getName(), "handle");
        if (JavaCGUtil.isCollectionEmpty(callGraphCallerInfoList)) {
            return;
        }

        for (CallGraphCallerInfo callGraphCallerInfo : callGraphCallerInfoList) {
            if (!MethodCallFlagsEnum.MCFE_METHOD_CALL_INFO.checkFlag(callGraphCallerInfo.getCallFlags())) {
                // 当前方法调用没有方法调用信息，跳过
                continue;
            }
            // 查询方法调用中被调用对象与参数使用的信息
            ObjArgsInfoInMethodCall objArgsInfoInMethodCall = methodCallInfoHandler.queryObjArgsInfoInMethodCall(callGraphCallerInfo.getCallId());
            // 获取第1个参数对应的参数信息
            ...
            for (MethodCallInfo methodCallInfo : methodCallInfoList) {
                ...
                methodCallHandler.manualAddMethodCall(callGraphCallerInfo.getCallerFullMethod(), calleeFullMethod);
            }
        }
    }
}
