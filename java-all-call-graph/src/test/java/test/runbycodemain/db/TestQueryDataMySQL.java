 ...
 * @date 2026/6/14
 * @description: 测试 DbOperWrapper 查询任意数据功能（MySQL数据库）
 */
@RunWith(JUnit4ClassRunnerSortMethod.class)
@JACGExample(title = "查询任意数据（MySQL数据库）",
        desc = {})
public class TestQueryDataMySQL {
    ...
    public void testQueryDataWithParam() {
        ...
        try (DbOperator dbOperator = dbOperWrapper.getDbOperator()) {
            String sql = "SELECT TABLE_NAME, TABLE_COMMENT FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME LIKE ? LIMIT ?";
            ...
            for (Map<String, Object> row : result) {
                System.out.println("  TABLE_NAME: [" + row.get("TABLE_NAME") + "] TABLE_COMMENT: [" + row.get("TABLE_COMMENT") + "]");
            }
        } catch (Exception e) {
    ...
    // 测试MySQL数据库方言下的Druid SQL解析是否正确拒绝非SELECT语句
    @Test
    public void testQueryDataWithTimeoutNonSelect() {
        ...
        try (DbOperator dbOperator = dbOperWrapper.getDbOperator()) {
            ...
            // MySQL特有的SHOW语句（非SELECT）
            QueryDataResult result6 = dbOperWrapper.queryDataWithTimeout("SHOW TABLES", null, 5);
            // SHOW语句在MySQL方言下被Druid解析为SHOW类型（非SELECT），应被拒绝
            Assert.assertFalse("SHOW应被拒绝", result6.isSuccess());

            System.out.println("DELETE失败信息: " + result1.getErrorMsg());
            System.out.println("混合语句失败信息: " + result5.getErrorMsg());
            System.out.println("SHOW失败信息: " + result6.getErrorMsg());
        } catch (Exception e) {
            Assert.fail(e.getMessage());
        }
    }
