 ...
 * @description:
 */
public class TestGenCallGraph4EeElEeClassName extends TestElRunByCodeBase {
    @Override
    ...
    protected String chooseElText() {
        return CommonElAllowedVariableEnum.EAVE_MC_EE_CLASS_NAME.getVariableName() + " == '" + TestElExampleCallee1.class.getName() + "'";
    }

    @Override
    protected String chooseTitle() {
        return "生成向上方法完整调用链判断被调用类名";
    }

    @Override
    protected String chooseDesc() {
        return "在生成向上方法完整调用链时，判断被调用类名是否等于指定关键字，忽略匹配的方法调用";
    }
