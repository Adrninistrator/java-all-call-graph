 ...
 * @description:
 */
public class TestGenCallGraph4EeElErPackageNameEndsWithIC extends TestElRunByCodeBase {
    ...
    protected String chooseElText() {
        return "string.endsWithIC(" + CommonElAllowedVariableEnum.EAVE_MC_ER_PACKAGE_NAME.getVariableName() + ", '" + TestGenCallGraph4EeElErPackageName.ER_PACKAGE_NAME_UPPER + "')";
    }

    @Override
    protected String chooseTitle() {
        return "判断以关键字结尾（忽略大小写）";
    }

    @Override
    protected String chooseDesc() {
        return "在生成向上方法完整调用链时，判断调用类包名是否以指定关键字结尾（忽略大小写），忽略匹配的方法调用";
    }
