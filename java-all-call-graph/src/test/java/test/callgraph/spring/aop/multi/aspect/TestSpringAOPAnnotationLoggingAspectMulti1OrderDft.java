...
@Aspect
@Component
public class TestSpringAOPAnnotationLoggingAspectMulti1OrderDft {
    private static final Logger logger = LoggerFactory.getLogger(TestSpringAOPAnnotationLoggingAspectMulti1OrderDft.class);
    ...
    // 返回后通知
    @AfterReturning(pointcut = "execution(* test.callgraph.spring.aop.multi.service.*.*(..))", returning = "result")
    public void logAfterReturning(JoinPoint joinPoint, Object result) {
        logger.info("AfterReturning method: {}, result: {}", joinPoint.getSignature().getName(), result);
    }

    // 异常通知
    @AfterThrowing(pointcut = "execution(* test.callgraph.spring.aop.multi.service.*.*(..))", throwing = "error")
