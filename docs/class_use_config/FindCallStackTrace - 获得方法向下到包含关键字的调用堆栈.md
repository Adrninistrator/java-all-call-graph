# 1. FindCallStackTrace - 获得方法向下到包含关键字的调用堆栈
...
|配置文件|说明|
|---|---|
|_jacg_gen_all_call_graph/method_class_4caller.properties|(作用) 生成指定类调用或方法向下的方法完整调用链时的配置文件（每行指定一项配置，可指定多行）|
|_jacg_gen_all_call_graph/caller_graph_callee_arg_type_polymorphism.properties|(作用) 生成向下完整方法调用链时，指定哪些方法参数作为被调用对象涉及多态时的类型替换（每行指定一项配置，可指定多行）|
|_jacg_business_data_type/business_data_type_show_4er.properties|生成向下的完整方法调用链时，需要显示的业务功能数据类型。若不指定则不显示业务功能数据|

### 1.2.3. 使用的区分顺序的其他配置参数

|配置文件|说明|
|---|---|
|_jacg_find_stack_keyword/find_stack_keyword_4er.properties|生成向下的方法完整调用链文件后，从最顶层调用方法开始向下查找包含指定关键字的方法的调用堆栈时，使用的关键字|
|_jacg_extensions/method_annotation_formatter.properties|在此定义处理方法上的注解生成用于显示信息的扩展类完整类名（每行指定一项配置，可指定多行）|
|_jacg_extensions/find_stack_keyword_filter.properties|在此定义用于对完整调用链文件生成调用堆栈时使用的过滤器扩展类完整类名，若未指定则使用配置参数中的关键字（每行指定一项配置，可指定多行）|
